...
> **代码事实补丁（2026-04-24）**：当前代码自审为 **73 ADR / 28 条不变式 / 21 migrations / 84 services / 72 scripts**，本地测试基线为 **794 passed, 4 skipped**；本地已起步 **Phase 72：矛盾复核 / operator-gated unmerge**，并已新增 **Codex native skill**（`codex_skill/` + install/update/validate 脚本），本机安装与交互式显式中文触发已验证。
...
- **三支柱**：
  - **A 严格工程化** 9.95/10（73 ADR + 28 不变式 + 794/4 测试 + 反身能力）
  - **B 通用型 Skill** 9.8/10（Claude Skills / OpenAI Assistants / MCP 三路宿主 + plugin 扩展点）
  - **C 数字赛博生态圈** 9.7/10（tick + 神经网格 + 世界建模 + Agent 自主 + 年度真跑证据）
- **Codex 状态**：本机 `~/.codex/skills/we-together` 已安装原生 skill；交互式 Codex 在 `~` 下对显式中文请求已能自动进入 `we-together` 语境。`codex exec` 仍不适合需要 MCP 审批的调用。
...
| cli VERSION | `0.19.0` |
| pytest 基线 | **794 passed + 4 skipped**，~34s 本机 |
...
.venv/bin/python -m pytest -q         # 期望 794 passed, 4 skipped
...
### 如果用户只说"继续推进" → 你需要：
1. 跑 `pytest -q` 确认 **794 passed / 4 skipped** 基线保持
