...
> **代码事实补丁（2026-04-23）**：当前代码自审为 **73 ADR / 28 条不变式 / 21 migrations / 84 services / 69 scripts**，本地测试基线为 **779 passed, 4 skipped**；本地已起步 **Phase 72：矛盾复核 / operator-gated unmerge**，`sqlite_vec/faiss` 已从 stub 升级为真 backend，tenant CLI 覆盖面已大幅扩展。
...
- **三支柱**：
  - **A 严格工程化** 9.95/10（73 ADR + 28 不变式 + 779/4 测试 + 反身能力）
...
| cli VERSION | `0.19.0` |
| pytest 基线 | **779 passed + 4 skipped**，~36s 本机 |
...
.venv/bin/python -m pytest -q         # 期望 779 passed, 4 skipped
...
| **v0.19.0** | **779** | **真向量后端 + 联邦写路径 + 年运行审计 + 多租户基线 + 本地发布前基线收口** |
...
### 如果用户只说"继续推进" → 你需要：
1. 跑 `pytest -q` 确认 **779 passed / 4 skipped** 基线保持
