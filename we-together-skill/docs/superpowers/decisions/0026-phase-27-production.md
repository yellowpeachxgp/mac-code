...
### D1. PyPI 发布基础设施
`.github/workflows/publish.yml`：`push tag v*` 触发 build + `twine upload`（需 secret `PYPI_TOKEN`）。`docs/publish.md` 完整 checklist 含 TestPyPI 流程；`docs/release_notes_template.md` 模板。
