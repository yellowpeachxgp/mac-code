...
9. **benchmark 扩展**: 跑一份 `bench_scale.py --n 10000` 归档
10. **测试补充**: 某个 service 覆盖 <90%，补 property-based 测试

## 🔴 半天级（中等）

11. **真 sqlite-vec 集成**: `VectorIndex(backend='sqlite_vec')` 把 stub 换成真跑
12. **federation 写路径**: POST `/memories` 添加一条 shared memory（经 token 鉴权）
...
18. **sqlite-vec + FAISS 双 backend + 100k 压测**
...
5. 新增代码必须通过全量 pytest + 不违反 27 条不变式
